...
class GraphLoader(object):
    def __init__(self, data_path, feature_meta, test_p, cuda_num=None, logger=None):
    ...
    def _load_data(self, indexes, nbatch):
        ...
        for i in range(0, int(len(inputs) / nbatch) * nbatch, nbatch):
            ...
            data, labels = Variable(torch.FloatTensor(data)), Variable(torch.LongTensor(np.where(labels)[2]))
